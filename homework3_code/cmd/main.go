...
func main() {
	...
	proof := flag.String("p", "pow", "\"pow\" or \"pos\" [chain mode param]")
	...
	if *proof != "pow" && *proof != "pos" {
                flag.Usage()
		return
        }	

	if *command == "chain" {
		runblockchain(listenF, target, seed, secio, suffix, initAccounts, datadir, proof)
...
func runblockchain(listenF *int, target *string, seed *int64, secio *bool, suffix *string, initAccounts *string, datadir *string, proof *string) {
	if *datadir == ""{
	...
	genesisBlock = blockchain.Block{0, t.String(), 0, blockchain.CalculateBlockHash(genesisBlock), "", 100, blockchain.Difficulty, "", "", make(map[string]blockchain.Transaction), defaultAccounts}
