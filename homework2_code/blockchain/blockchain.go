...
// validators keeps track of open validators and balances, used for PoS.
var validators = make(map[string]int)

// pickWinner creates a lottery pool of validators and chooses the validator who gets to forge a block to the blockchain
// by random selecting from the pool, weighted by amount of tokens staked
func ContinuePickWinner(rw *bufio.ReadWriter) {
  for {
	...
	OUTER:
		...
		// Deterministically randomly pick winner from lottery pool
		...
		for _, block := range temp {
			if block.Validator == lotteryWinner {
				log.Printf("Winning validator : %s\n", lotteryWinner)
				Lock()
...
func MakeBasicHost(listenPort int, secio bool, randseed int64) (host.Host, error) {
	...
	log.Printf("I am %s\n", fullAddr)
	if secio {
		log.Printf("Now run \"go run main.go -c chain -l %d -d %s -p %s -secio\" on a different terminal\n", listenPort+2, fullAddr, BlockchainInstance.Proof)
	} else {
