...
func main() {
	...
	proof := flag.String("p", "pow", "[\"pow\" or \"pos\"]")
	...
	if *command == "chain" {
		runblockchain(listenF, target, seed, secio, suffix, initAccounts, proof)
...
func runblockchain(listenF *int, target *string, seed *int64, secio *bool, suffix *string, initAccounts *string, proof *string){
	t := time.Now()
	...
	genesisBlock = blockchain.Block{0, t.String(), 0, blockchain.CalculateBlockHash(genesisBlock), "", 100, blockchain.Difficulty, "", "", nil, defaultAccounts}
