...
// Decay reward every 50 blocks, mimicing bitcoin reward system.
const RewardDecayBlockInterval = 50
// Assume penalty is deducting the same amount as reward.
...
func ReadData(rw *bufio.ReadWriter) {
	for {
		...
		if str != "\n" {
			...
			has_diff := false
			mutex.Lock()
			has_diff = ProcessBlocksDiff(tMessage.Blocks) || ProcessTransactionsDiff(tMessage.PendingTransactions)
			if has_diff {
				PrintBlockchain()
	...
	for {	
		...
		if BlockchainInstance.Proof == "pos" || BlockchainInstance.Proof == "pox" {
			...
			if blockValid {
				...
				rewardDeductRatio := 1
				if newBlock.Index > RewardDecayBlockInterval {
					rewardDeductRatio = newBlock.Index / RewardDecayBlockInterval
				}
				mutex.Lock()
				newBlock.MinerReward += uint64(NewBlockReward / rewardDeductRatio)
